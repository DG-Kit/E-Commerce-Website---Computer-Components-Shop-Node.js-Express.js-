...
const ProductAdd = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        // Use query parameter to get categories that are child categories (level > 0)
        const response = await adminApi.getCategories({ parent: 'all', flat: 'true' });
        if (response.data?.data) {
          // Filter to only show child categories (those with a parent)
          const childCategories = response.data.data.filter(category => category.parent !== null);
          setCategories(childCategories);
          console.log('Categories loaded:', childCategories);
        } else if (response.data) {
          // If the API doesn't return data in a data property, try the response.data directly
          const childCategories = response.data.filter(category => category.parent !== null);
          setCategories(childCategories);
          console.log('Categories loaded (alt):', childCategories);
        } else {
          console.error('Unexpected API response format:', response);
          setError('Định dạng dữ liệu danh mục không đúng. Vui lòng thử lại sau.');
        }
      } catch (error) {
  ...
  // Add automatic price and stock calculation function 
